...
# TODO(SED): Remove this block when the TPM bank 3 will be added
SED_DEFAULT_PASSWORD="eGN8W0VlNGtYKmFcYy82ViY1N2ReZVAhelVPS10tKjAK"
sed_pw_default=$(echo "$SED_DEFAULT_PASSWORD" | base64 -d)
if [ -z "$sed_pw_default" ]; then
    log_error "Default SED password is not defined"
    exit 1
fi

# TODO(SED): Remove the comment when the TPM bank 3 will be added
